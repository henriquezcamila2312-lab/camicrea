...
- **Dónde aparece:** Sección "Portafolio" (cada tarjeta es una carpeta del archivador y usa `.marco-foto.marco-foto--abstracto`, un fondo abstracto de textura y color, sin ningún texto sobre la imagen). Para conectar una foto real, en el `<div class="marco-foto marco-foto--abstracto" aria-hidden="true"></div>` de la tarjeta correspondiente: quita `aria-hidden="true"` y la clase `marco-foto--abstracto`, y agrega adentro una `<img src="assets/proyecto-oncomamas.jpg" alt="...">` (con `loading="lazy"` si no es la primera imagen visible de la página). El marco (`.marco-foto`) ya reserva el espacio y el borde redondeado, así que no hace falta tocar el resto del HTML ni el CSS.
...
## 11. Videos de flores — guardados, sin usar por ahora

Camila envió dos videos verticales (1080 × 1920 px): `animaciones/flores-01.mp4` y
`animaciones/flores-02.mp4`. Se probaron en Inicio (junto al botón "Explora mi
trabajo") pero se quitaron porque no se veían bien en ese lugar. Los dos
archivos quedan guardados en `assets/animaciones/` sin usarse en ninguna
página. Avísame si quieres que los use en otro lugar del sitio y con qué
tratamiento (tamaño, marco, dónde).
